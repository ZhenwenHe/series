...
public class KmeansClusterer<S extends Series, L extends String> extends DefaultClusterer<S,L>{
    ...
    private List<S> centralIteration(List<S> central, List<L> clusterLabelList) throws IOException {
        ...
        for (int i = 0; i < k; i++) {
            ...
            long len = this.testSet.getSample(0).length();
            double[] xData = new double[(int)len];
            double[] yData = new double[(int)len];
            for (int j = 0; j < clusterIndex.size(); j++) {
                double[] tmpX = this.testSet.getSample(clusterIndex.get(j)).getDataX();
                double[] tmpY = this.testSet.getSample(clusterIndex.get(j)).getDataY();
                for (int l = 0; l < len; l++) {
                    xData[l] += tmpX[l];
                    yData[l] += tmpY[l];
                }
            ...
            seriesBuilder.addFieldValues(yData);
            seriesBuilder.addTimeValues(xData);
            Series tmp = seriesBuilder.build();
